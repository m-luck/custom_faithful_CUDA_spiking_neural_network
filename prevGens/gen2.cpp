...
const uint synapses=100;//synapses per neuron
...
uint spike[hz][2];//Timing of spikes, max of hz limit
...
uint ps_set[total][synapses];//Matrix holding the post-synaptic neurons(synapses) of each neuron(total)
...
uint delays_length[total][delay];//Matrix holding the delay values of each neuron
uint del_set[total][delay][synapses];//Matrix holding the delays to each synapse from each neuron
uint pre_neuron[total];//Index of presynaptic information
uint pre_input[total][synapses*3];//Presynaptic inputs
uint pre_delay[total][synapses*3];//Presynaptic delays
